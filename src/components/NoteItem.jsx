    ...
    >
        ...
        <div className="flex text-xs justify-between">
          ...
          <p className="flex items-center gap-1">
            ...
            <span className="bg-green-100 text-green-800 px-2 rounded-md">
              {`${convertToMinutes(item?.callDuration)}:${convertToSecons(
                item?.callDuration
              )}`}
            </span>
          </p>
      ...
      <div className="px-4 py-2">
        <p className=" flex items-center gap-1 text-xs mb-1">
          ...
          <span className="bg-purple-100 text-neutral-800 capitalize px-2 rounded-md">
            {item?.createdBy?.userName || "N/A"}  {`( ${roles.getRoleNameById(item?.createdBy?.role)} )`  }
          </span>
        </p>
