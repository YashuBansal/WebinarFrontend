...
export const employeeSlice = createSlice({
  ...
  reducers: {
    ...
    clearEmployeeData(state) {
      console.log('clearing emp data')
      state.employeeData = [];
    },
    clearSingleEmployeeData(state) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getAllEmployeesSilently.fulfilled, (state, action) => {
        state.employeeData = action.payload?.result || [];
        state.totalPages = action.payload?.totalPages;
      })
      .addCase(getAllEmployeesSilently.rejected, (state, action) => {
        errorToast(action?.payload);
      })
...
export const { clearSuccess, setEmployeeModeId, clearEmployeeData } = employeeSlice.actions;
