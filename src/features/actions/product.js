...
export const getEnrollmentsByEmail = createAsyncThunk(
  "enrollments/product-email-enrollments",
  async ({ email }, { rejectWithValue }) => {
    console.log("my email id in action",email)
    try {
...
export const getEnrollmentsByLevelOrId = createAsyncThunk(
  ...
  async (payload, { rejectWithValue }) => {
    try {
      const { data } = await instance.get(`/enrollments/level-or-product`, {
        params: payload,
      });
      return data;
    } catch (e) {
